...
def run_nested_cv(all_graphs, all_labels_encoded, label_encoder, num_classes, config):
    ...
    for outer_idx, (train_idx, test_idx) in enumerate(
        ...
        study   = optuna.create_study(
            ...
            study_name=f"w2v_{config['source_cpus'][0]}_outer{outer_idx}",
        )
...
def run_arch(arch, w2v_model, n_trials=None):
...
def main():
    parser = argparse.ArgumentParser(
        description="Single-arch family classification (Word2Vec, Nested CV)"
    )
    ...
    summaries = {}
    for arch in archs:
        summary = run_arch(arch, args.w2v_model, n_trials=args.n_trials)
        if summary:
