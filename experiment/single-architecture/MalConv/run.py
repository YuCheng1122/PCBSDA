...
def build_scheduler(optimizer, config):
    return torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=config.get("cosine_T_max", 50)
    )
...
def run_nested_cv(file_names, labels, label_encoder, num_classes, config):
    ...
    for outer_idx, (train_idx, test_idx) in enumerate(
        ...
        study   = optuna.create_study(
            ...
            study_name=f"malconv_{config['source_cpus'][0]}_outer{outer_idx}",
        )
        objective = make_objective(outer_train_names, outer_train_y, num_classes, config)
...
def main():
    parser = argparse.ArgumentParser(
        description="Single-arch family classification (MalConv raw bytes, Nested CV)"
    )
