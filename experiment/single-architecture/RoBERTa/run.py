"""
Single-Architecture Family Classification — RoBERTa Embedding
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__)))
from config import get_roberta_single_config, ALL_ARCHS, ROBERTA_TAGS
...
def run_arch(arch, roberta_tag, tune_only=False, eval_only=False, n_trials=None):
    ...
    print(f"\n{'='*60}")
    print(f"Arch: {arch} | Embedding: {roberta_tag}")
    print(f"{'='*60}")
