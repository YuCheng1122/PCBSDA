"""
Cross-architecture CCSA experiments.
...
Each pair runs 3 random seeds. MIPS is test-only (never source).
...
def run_experiment(config, random_state, dirs, logger):
    ...
    for epoch in range(1, epochs + 1):
        ...
        tgt_train_acc, _       = evaluate(model, target_train_loader, device)
        ...
        logger.info(f"{epoch:>6}  {train_loss:>8.4f}  {train_ce:>8.4f}  {train_csa:>8.4f}  "
                    f"{val_accuracy:>10.4f}  {tgt_train_acc:>10.4f}  {tgt_accuracy:>11.4f}")
...
def run_pair(base_config, src_cpus, tgt_cpus, seeds, device):
    ...
    # Per-pair output dirs: append /<tag> to each base dir from config
    ...
    logger = setup_logger(cfg["log_dir"], name=f"ccsa_{tag}")
    ...
    logger.info(f"=== CCSA | {src_cpus} -> {tgt_cpus} ===")
...
def main():
    ...
    print(f"\n{'#'*60}")
    print(f"# CCSA Cross-Architecture — All Pairs")
    print(f"# Seeds: {seeds}")
    print(f"# Pairs:")
