...
from configs.dsne.baseline import get_dsne_config
from src.transfer_learning.dsne.models import GCN_DSNE, GAT_DSNE
from src.transfer_learning.dsne.utils import (
    prepare_dsne_data, dsne_loss, train_dsne_epoch,
    evaluate, test_model,
    plot_training_curves, save_experiment_results
)


def setup_logger(log_dir, name="dsne"):
...
def build_model(config, num_classes, device):
    ...
    if model_type == "GAT":
        model = GAT_DSNE(**common_kwargs, heads=config.get("gat_heads", 4))
    else:
        model = GCN_DSNE(**common_kwargs)
    return model.to(device)


def run_experiment(config, random_state, dirs, logger):
    # --- Load data ---
    src_train, src_val, tgt_train, tgt_test, label_encoder, num_classes = prepare_dsne_data(config, random_state)
    ...
    # target_batch: 全部 few-shot target 一次性打包，每個 step 都用這個
    device = config["device"]
    target_batch = Batch.from_data_list(tgt_train).to(device)
    ...
    alpha = config["alpha"]
    beta = config["beta"]
    dsne_margin = config["dsne_margin"]
    bidirectional = config.get("bidirectional", True)
    epochs = config["epochs"]
    patience = config["patience"]

    logger.info(f"\n--- Training (seed={random_state}) ---")
    logger.info(f"{'Epoch':>6}  {'Loss':>8}  {'CE_src':>8}  {'CE_tgt':>8}  {'dSNE':>8}  "
                f"{'SrcValAcc':>10}  {'TgtFewAcc':>10}  {'TgtTestAcc':>11}")
    logger.info("-" * 80)

    for epoch in range(1, epochs + 1):
        train_loss, train_ce_src, train_ce_tgt, train_dsne = train_dsne_epoch(
            model, src_loader, target_batch, optimizer, ce_criterion,
            device, alpha, beta, dsne_margin, bidirectional=bidirectional
        )
        ...
        logger.info(f"{epoch:>6}  {train_loss:>8.4f}  {train_ce_src:>8.4f}  {train_ce_tgt:>8.4f}  "
                    f"{train_dsne:>8.4f}  {val_accuracy:>10.4f}  {tgt_train_accuracy:>10.4f}  "
                    f"{tgt_accuracy:>11.4f}")
    ...
    save_path = os.path.join(dirs["model_output_dir"], f"dsne_best_rs{random_state}.pt")
...
def main():
    ...
    logger = setup_logger(dirs["log_dir"], name="dsne")
    ...
    logger.info(f"=== d-SNE Transfer Learning ===")
    ...
    logger.info(f"Alpha: {config['alpha']} | Beta: {config['beta']} | Margin: {config['dsne_margin']} | Bidirectional: {config.get('bidirectional', True)}")
